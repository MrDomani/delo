...
class TestAbstractDESetUp(unittest.TestCase):
    ...
    def set_get_example_population_trial(self):
        ...
        self.solver.population_trial = population_trial_org
...
class TestAbstractDEInit(TestAbstractDESetUp):
    ...
    def test_init_population(self):
        ...
        self.solver.init_population()
        self.assertIsInstance(self.solver.population, np.ndarray)
        self.assertTupleEqual(self.solver.population.shape,
                              (self.solver.population_size, self.solver.function.dimension))
        np.testing.assert_array_less(self.solver.population, np.tile(self.solver.function.domain_upper_limit + 1e-14,
        ...
        self.assertGreater(np.unique(self.solver.population).size, self.solver.population.size * 0.7)

    def test_set_p_best(self):
        self.solver.population_size = 100
        self.solver.population_f_value = np.linspace(0, 1, num=self.solver.population_size, endpoint=False)
        self.solver.p_best_rate = 0.1
        self.solver.set_p_best()
...
class TestAbstractDERestarts(TestAbstractDESetUp):
    ...
    def test_after_optimization(self):
        ...
        np.testing.assert_array_equal(self.restrigin5D(self.solver.population),
                                      self.solver.population_f_value)

        best_member_ever1, best_f_value_ever1 = self.solver.get_solution()
        np.testing.assert_array_equal(best_f_value_ever1, np.min(self.solver.population_f_value))
        np.testing.assert_array_equal(best_member_ever1,
                                      self.solver.population[np.argmin(self.solver.population_f_value)])
        ...
        self.assertTrue(
            self.solver.remaining_evals == 3)  # After 10 generations, 3 was left and it was less than 100, so optimization stopped
...
class TestAbstractDESetUpTrim(TestAbstractDESetUp):
    def setUp(self):
        super().setUp()
        self.solver.population = self.set_get_example_population_trial()
        self.solver.population_trial = self.set_get_example_population_trial() * 1.5
        population_trial_org = self.solver.population_trial

    def test_does_not_trim_when_not_necessary(self):
        population_trial_org = self.set_get_example_population_trial()
        self.solver.trim_population_trial_to_domain()
        np.testing.assert_array_equal(self.solver.population_trial, population_trial_org)
    ...
    def test_trims_in_specific(self):
        ...
        self.solver.population = np.zeros(population_trial_org.shape)
        self.solver.population_trial = population_trial.copy()
        self.solver.trim_population_trial_to_domain()

        expected_population_trial = np.where(population_trial == -10, -5, 0)
        np.testing.assert_array_equal(self.solver.population_trial, expected_population_trial)

    def test_trims_when_outside_domain(self):
        self.solver.trim_population_trial_to_domain()
        np.testing.assert_array_less(self.solver.population_trial,
                                     5 * np.ones(self.solver.population_trial.shape) + 1e-14)
        np.testing.assert_array_less(-5 * np.ones(self.solver.population_trial.shape) - 1e-14,
                                     self.solver.population_trial)
    ...
    def test_trims_when_necessary(self):
        ...
        np.testing.assert_array_equal(self.solver.population_trial[were_in_domain],
                                      population_trial_org[were_in_domain])

    def test_trims_linearly(self):
        original_delta_x = self.solver.population_trial - self.solver.population
        self.solver.trim_population_trial_to_domain()
        new_delta_x = self.solver.population_trial - self.solver.population
    ...
    def test_trims_as_little_as_possible(self):
        were_not_in_domain = np.logical_or(self.solver.population_trial <= -5,
                                           self.solver.population_trial >= 5).any(axis=1)
        self.solver.trim_population_trial_to_domain()
        min_dist_from_edge_of_domain = np.minimum((5 - self.solver.population_trial[were_not_in_domain]).min(axis=1),
                                                  (self.solver.population_trial[were_not_in_domain] + 5).min(axis=1))
...
class TestAbstractDESetUpProcessOptimization(TestAbstractDESetUp):
    def test_crossover(self):
        population_trial_org = self.set_get_example_population_trial()
        self.solver.CR = np.ones(shape=self.solver.population_size)
        self.solver.crossover()
        np.testing.assert_array_equal(self.solver.population_trial,
                                      population_trial_org)  # whole of population_trial_org should go, cos all CR = 1

        population_trial_org = self.set_get_example_population_trial()
        self.solver.CR = np.zeros(shape=self.solver.population_size)
        self.solver.crossover()
        np.testing.assert_array_equal((self.solver.population_trial == population_trial_org).sum(axis=1),
                                      np.ones(self.solver.population_size))  # in every row, exactly one equal

    def test_evaluate(self):
        population_trial_org = self.set_get_example_population_trial()
        self.solver.evaluate()
        np.testing.assert_array_equal(self.solver.population_trial_f_value,
                                      self.restrigin5D(population_trial_org))

    def test_set_delta_f_and_get_improvement_bool(self):
        self.solver.population_f_value = np.ones(self.solver.population_size)
        self.solver.population_trial_f_value = np.linspace(0, 2, num=self.solver.population_size)
        have_improved = self.solver.set_delta_f_and_get_improvement_bool()

        desired_delta_f = np.where(self.solver.population_trial_f_value <= 1, 1.0-self.solver.population_trial_f_value, 0)
        np.testing.assert_allclose(self.solver.delta_f, desired_delta_f)

        np.testing.assert_array_equal(have_improved, self.solver.population_trial_f_value <= 1)
